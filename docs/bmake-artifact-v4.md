...
Confit은 source TOML, declared component/nucleus/test root, typed override와 sibling
Makefile의 bounded API 3 data만 읽는다. Makefile을 실행하거나 C contents를 scan하지 않으며,
...
`component.catalog.json`은 selectable component schema v3 catalog와 feature/KAPI reason
graph를 싣는다. Reason은 source path와 line/column을 포함하며 consumer가
provider를 다시 추론하거나 first-match fallback을 수행하지 못하게 한다.

`tests.mk`는 중앙 test ID registry를 대체한다. `PARUS_TEST_IDS`는 bounded
`test_roots`에서 발견한 모든 owner-local `.include <parus.test.mk>` Makefile을 결정적으로 열거하고,
각 ID에 대해 owner, lane, evidence class, nonzero timeout, source directory와 sealed
source list를 낸다. Test owner는 exact nucleus unit, selectable component 또는
`target.<TEST_TARGET>` identity여야 하고 lane/evidence 조합은 closed compatibility
table과 일치해야 한다. Test는 selectable component kind가 아니다.

`nucleus.mk`는 declared `nucleus_roots`와 bounded `KERN_SUBDIRS` traversal에서 얻은
