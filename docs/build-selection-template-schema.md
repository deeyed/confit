...
Section 이름과 field 이름은 project가 정한다. Confit은 의미를 해석하지
않고, field value가 기존 option id를 가리키는지만 검증한다.
...
이 문서는 template loader 계약만 고정한다. 실제 Lua/QSM, JSON, CMake
build-selection artifact 생성은 별도 generator 라운드에서 구현한다.
