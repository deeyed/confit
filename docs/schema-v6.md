...
bounded single-line string으로 소유하고 layout/control byte를 거부한다. R11은 catalog가
완성된 뒤 모든 expression을 parse, link, type-check하고 cycle을 거부한 immutable
dependency plan까지 schema project에 소유시킨다. 구체적인 경계는
`docs/expression-v6.md`에 기록한다.
