...
기본 `Confit CI` workflow는 push와 pull request마다 다음 platform을 함께
검증한다.
...
Windows job은 CLI-only preview gate다. 이 job은 CMake configure 시
`-DCONFIT_ENABLE_TUI=ON`을 일부러 전달한 뒤, Windows guard가
`CONFIT_ENABLE_TUI:BOOL=OFF`로 강제했는지 확인한다.

Windows push CI에서 보장하는 항목:

- GNU-style clang + Ninja build
- CTest 실행
- C 기반 CLI integration test
- `confit doctor`의 `windows-cli-only`, `tui: unsupported`,
  `<prefix>/bin/confit.exe` install rule 출력
- `confit check`
- `confit gen --artifact all`
- `config.h`, `config.cmake`, `config/config.qsm`, `config.qst`,
  `delos_build_selection/delos_build_selection.qsm`, report/input/graph/explain
  artifact 존재
- `confit tui`가 partial UI 없이 exit code `8`로 실패하는지 확인
- CMake install smoke로 `install-windows-ci/bin/confit.exe` 생성 확인

`Confit Windows CLI Preview` 수동 workflow는 남겨두지만, 정식 preview gate는
push/pull request `Confit CI`의 `windows-clang-cli` job이다.
