...
## End-To-End Delos Example

Delos release profile과 Renode Nucleo-H753ZI target을 생성하는 정본 예시는
다음과 같다. `fixtures/delos`는 Delos 팀이 project fixture 또는 실제 config
source를 그 경로에 둘 때의 canonical example path다. 현재 Confit repository
안의 realish fixture는 `tests/fixtures/realish/delos`다.
...
QStar graph는 pure config module과 build selection module을 분리해서 읽는다.
...
CMake graph는 같은 output directory의 `config.cmake`를 include한다.
...
실제 adoption 절차와 review gate는 `docs/build-selection-workflow.md`를
정본으로 따른다.
