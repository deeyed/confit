...
| `--artifact header|reports|cmake|qstar|all` | Select generated artifact groups. The `qstar` group emits canonical `config/config.qsm`, project-specific build selection modules declared by `selection/*.toml`, and any documented compatibility artifact. |
