...
Confit은 Parus의 **host-side configuration resolver**다. 사람이 작성한 generic project
schema v2 TOML, selectable component/profile schema v3와 restricted Make API 3을
검증·해결하고, immutable selection snapshot을 sealed
artifact ABI v4 bundle로 publish한다. Confit은 kernel, firmware, runtime service 또는
build graph executor가 아니다.

## 고정 경계

- generic project/option/target source는 `schema_version = 2`, selectable component와
  selection profile은 `schema_version = 3`만 허용한다. 두 authority의 converter,
  migration command와 compatibility dispatch는 없다.
...
- `gen`은 `--artifact bundle`만 허용한다. partial artifact 또는 backend selector는
  fail-closed한다.
- Confit은 sibling component/nucleus/test Makefile을 실행하지 않고 API 3의 literal
  assignment와 마지막 public include만 bounded data grammar로 해석한다. 명시된 source의 존재·소유권을
