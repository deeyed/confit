  ...
  if (status == CONFIT_OK) {
    ...
    for (index = 0U; index < 2U; ++index) {
      ...
      if (!confit_catalog_config_at(catalog, index, &view) ||
        ...
        while (index > 0U) confit_value_destroy(&values[--index]);
...
int main(void) {
  static const char *const corpus[] = {
      "",          "BASE",       "!BASE",       "BASE && BASE",
      "BASE || BASE", "(BASE)",  "BASE == true", "BASE != false",
      "UNKNOWN",   "BASE + BASE", "((((BASE))))", "BASE == \"x\"",
  };
  ...
  for (index = 0U; index < sizeof(corpus) / sizeof(corpus[0]); ++index)
    if (!compile_one(corpus[index])) return 2;
  for (index = 0U; index < 2048U; ++index) {
    char generated[257];
    size_t length = (size_t)(next_state(&state) % 256U);
    size_t cursor;
    for (cursor = 0U; cursor < length; ++cursor)
      generated[cursor] = alphabet[next_state(&state) %
                                   (sizeof(alphabet) - 1U)];
    generated[length] = '\0';
    if (!compile_one(generated)) return 3;
  }
  return 0;
}
