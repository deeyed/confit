...
6. config.cmake, config/config.qsm, build selection module 연결
7. rollback 절차 검증
```
...
Delos board/CPU/linker selection까지 포함한 fixture rehearsal:
...
## 6단계: CMake/QStar/build selection 연결

Confit은 `config.cmake`와 QStar용 canonical pure module
`config/config.qsm`, project-specific build selection module을 만들지만,
build graph를 자동으로 수정하지 않는다. 기존 `config.qst`는 compatibility
artifact로 남는다. 실제 연결은 별도 review에서 한다.
...
`selection.board.objects`는 `qstar.config`가 아니라 실제 target의
`objects = { ... }` 또는 target-local field에서 소비한다.
...
- QStar graph가 board object/include/linker 선택을 build selection module에서 읽는가.
- CMake graph가 같은 output directory의 `config.cmake`를 include하는가.
- rollback이 단순한가.
