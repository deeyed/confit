...
ConfitStatus confit_v4_generate_artifacts(
  ...
  ConfitV4NamedText identity_texts[11];
  ...
  if (status == CONFIT_OK) {
    ...
    identity_texts[10] = (ConfitV4NamedText){"component.catalog.json", out_artifacts->component_catalog_json, {0}};
    ...
    if (status == CONFIT_OK) {
      ConfitV4NamedText manifest_texts[12];
      memcpy(manifest_texts, identity_texts, sizeof(identity_texts));
      manifest_texts[11] = (ConfitV4NamedText){"config.mk", out_artifacts->config_mk, {0}};
...
static ConfitStatus confit_v4_complete_texts(
    const ConfitV4ArtifactSet *artifacts, ConfitV4NamedText texts[13],
    ConfitDiagnostic *diagnostic) {
  ...
  texts[11] = (ConfitV4NamedText){"component.catalog.json", artifacts->component_catalog_json, {0}};
  texts[12] = (ConfitV4NamedText){"config.bundle.json", artifacts->bundle_json, {0}};
  return CONFIT_OK;
}

static ConfitStatus confit_v4_verify_generation(
    const char *generation, const ConfitV4NamedText texts[13],
    ConfitDiagnostic *diagnostic) {
  size_t index;
  ConfitStatus status = CONFIT_OK;
  char path[4096];

  for (index = 0U; status == CONFIT_OK && index < 13U; ++index) {
...
static ConfitStatus confit_v4_write_staging(
    const char *staging, const ConfitV4NamedText texts[13],
...
static void confit_v4_mark_read_only(const char *generation,
                                     const ConfitV4NamedText texts[13]) {
#if !defined(_WIN32)
  char path[4096];
  size_t index;
...
static ConfitStatus confit_v4_publish_artifacts_posix(
    ...
    const ConfitV4NamedText texts[13], size_t *out_changed_file_count,
    ConfitDiagnostic *diagnostic) {
